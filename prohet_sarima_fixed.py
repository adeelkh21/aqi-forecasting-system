...
class EnhancedAQIForecaster:
    ...
    def _evaluate_lstm_model(self, model, data: pd.DataFrame) -> float:
        ...
        try:
            ...
            if not hasattr(self, 'lstm_scaler_X') or not hasattr(self, 'lstm_scaler_y') or not hasattr(self, 'lstm_features'):
                print("   ❌ LSTM scalers not available for evaluation")
                return -999.0
            
            # Prepare data using the same features and scaling as training
            X = data[self.lstm_features].values
            
            # Handle any missing features
            missing_features = set(self.lstm_features) - set(data.columns)
            if missing_features:
    ...
    def forecast_72_hours(self, location: Dict) -> Dict:
        ...
        try:
            ...
            if not hasattr(self, 'models') or not self.models:
                print("   ⚠️ Models not trained yet. Training models first...")
            ...
            # Create future timestamps (next 72 hours)
            last_timestamp = latest_data['date'].iloc[-1]
                ...
                if name in self.models and self.models[name]['model'] is not None:
                    try:
                        if name == 'lstm':
                            ...
                            if hasattr(self, 'lstm_features') and hasattr(self, 'lstm_scaler_X') and hasattr(self, 'lstm_scaler_y'):
    ...
    def _normalize_data_for_lstm(self, train_data: pd.DataFrame, val_data: pd.DataFrame, test_data: pd.DataFrame = None):
        ...
        try:
            ...
            self.lstm_features = important_features
